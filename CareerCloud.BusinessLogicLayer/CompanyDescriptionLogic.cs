   ...
   public class CompanyDescriptionLogic : BaseLogic<CompanyDescriptionPoco>
    {
        ...
        protected override void Verify(CompanyDescriptionPoco[] pocos)
        {
            ...
            {
                if(string.IsNullOrEmpty(poco.CompanyDescription) ||  poco.CompanyDescription.Trim().Length <= 2)
                {
                    exceptions.Add(new ValidationException(107, "Company Description must be greater than 2 characters"));
                }

                if(string.IsNullOrEmpty(poco.CompanyName) || poco.CompanyName.Trim().Length <= 2 )
                {
